...
//   2.  Issue material — calc modes: server | sqft | dimensions | outsource
//       + quantity-based products (pcs, rolls, etc.) — uses "dimensions" mode
//         with zeroed sqft fields; product_type="quantity" flag stored in notes
...
//
// calc_mode values accepted (must match existing model enum):
//   "server"     — legacy: width/height → server calculates sqft
//   "sqft"       — flat sqft number from cart
//   "dimensions" — printing + media dimensions (sqft product)
//   "outsource"  — outsourced; optional dims for reference, no stock deduction
//
// Quantity-based products (product_type === "quantity") also use "dimensions"
// but with all sqft fields zeroed. issued_qty is in the product's own unit
// (pcs, rolls, etc.). This avoids needing a schema migration.
// ─────────────────────────────────────────────────────────────────────────────
const buildCalculation = ({
  ...
  const isQtyProduct  = !isOut && product_type === "quantity";

  // Quantity-based products: store unit counts, all sqft fields = 0.
  // We use effectiveMode "dimensions" so it passes schema enum validation
  // (no schema migration required).
  if (isQtyProduct) {
    const qty    = parseFloat(issued_qty) || 0;
    const refQty = parseFloat(job_qty)    || 0;
    const extra  = parseFloat(Math.max(0, qty - refQty).toFixed(4));

    const calc = {
      // sqft fields — all zero for quantity products
      job_sqft:             0,
      margin_sqft:          0,
      gross_sqft:           0,
      print_sqft:           0,
      ...
      buffer_sqft:          0,
      // required_sqft repurposed as "required_qty" for quantity products
      required_sqft:        qty,
      margin_top_inches:    0,
    ...
    return {
      calc,
      dimensionRecord: { width: 0, height: 0, unit: "pcs" },
      printDim:        null,
      mediaDim:        null,
      wastageSq:       0,
      effectiveMode:   "dimensions", // valid enum value — no schema change needed
      isQtyProduct:    true,
      job_qty:         refQty,
      extra_qty:       extra,
    };
  ...
  // Determine effective mode for sqft products
  ...
  if (effectiveMode === "outsource") {
    const pFtW = printing_dimensions
      ? toFeet(printing_dimensions.width,  printing_dimensions.unit || "ft") : 0;
    const pFtH = printing_dimensions
      ? toFeet(printing_dimensions.height, printing_dimensions.unit || "ft") : 0;
    const mFtW = media_dimensions
      ? toFeet(media_dimensions.width,  media_dimensions.unit || "ft") : 0;
    const mFtH = media_dimensions
      ? toFeet(media_dimensions.height, media_dimensions.unit || "ft") : 0;
    ...
    if (printing_dimensions)
      printDim = {
        width:  printing_dimensions.width,
        height: printing_dimensions.height,
        unit:   printing_dimensions.unit || "ft",
        sqft:   pSq,
      };
    if (media_dimensions)
      mediaDim = {
        ...
        unit:   media_dimensions.unit || "ft",
        sqft:   mSq,
      };
    wastageSq = parseFloat(Math.max(0, mSq - pSq).toFixed(4));

    return { calc, dimensionRecord, printDim, mediaDim, wastageSq, effectiveMode, isQtyProduct: false };
  }

  // ── "dimensions" mode — sqft product with printing + media dims ───────────
  if (effectiveMode === "dimensions") {
    const pd    = printing_dimensions || {};
    const md    = media_dimensions    || {};
    const pUnit = pd.unit || "ft";
    const mUnit = md.unit || "ft";
  ...
  // ── "sqft" mode — flat sqft number from cart ──────────────────────────────
  if (effectiveMode === "sqft") {
    const cartSqFt = parseFloat(sq_ft) || 0;
    ...
    if (dimensions)
      dimensionRecord = { width: dimensions.width || 0, height: dimensions.height || 0, unit: "ft" };
    return { calc, dimensionRecord, printDim, mediaDim, wastageSq, effectiveMode, isQtyProduct: false };
  }

  // ── "server" mode — legacy: width/height → server calculates sqft ─────────
  {
...
//
//   SQFT products  (product_type omitted, or "sqft"):
//     calc_mode "dimensions" → requires printing_dimensions + media_dimensions
//     calc_mode "sqft"       → requires sq_ft
//     calc_mode "server"     → requires dimensions.width + dimensions.height
//
//   QUANTITY products  (product_type === "quantity", e.g. pcs / rolls):
//     No dimension fields required.
//     issued_qty = units to deduct from stock.
//     job_qty    = units the job needs (reference only, stored for reporting).
//     calc_mode saved as "dimensions" with all sqft fields = 0.
//     No PDF slip generated (sqft-only feature).
// =============================================================================
exports.issueMaterial = async (req, res) => {
  try {
    ...
    const {
      ...
      product_type        = "sqft",  // "sqft" | "quantity"
      job_qty             = null,    // reference qty for quantity products
    } = req.body;

    const isOutsourced = outsource_type && outsource_type !== "none";
    const isQtyProduct = !isOutsourced && product_type === "quantity";
    ...
    if (isQtyProduct) {
      const qtyNum = parseFloat(issued_qty) || 0;
      if (qtyNum <= 0)
        return resp(res, 400, false, "issued_qty must be greater than 0 for quantity-based products.");
    }

    // ── sqft product dimension validation ─────────────────────────────────
    if (!isOutsourced && !isQtyProduct) {
    ...
    if (!isOutsourced && material?.product_id) {
      ...
      const unit      = product.primary_unit || "sqft";

      if (available < qtyNeeded)
        return resp(res, 400, false,
          `Insufficient stock for "${product.name}". Available: ${available} ${unit}, Requested: ${qtyNeeded} ${unit}`
        );
    }
    ...
    if (isOutsourced) {
      ...
      if (issued_to?.user_id) {
        ...
        const designFile = cartItem?.design_files?.find(
          f => f._id?.toString() === design_file_id
        );
      ...
      if (resolvedIssuedTo.user_id) {
        ...
        if (!employee)
          console.warn(`issueMaterial: employee ${resolvedIssuedTo.user_id} not found`);
      }
    }

    // ── Build calculation snapshot ────────────────────────────────────────
    const calcResult = buildCalculation({
      calc_mode,
      sq_ft,
      dimensions,
      printing_dimensions,
    ...
    // For quantity products use the product's primary_unit (pcs, rolls, etc.)
    // For sqft products use "sqft"
    // For outsource use whatever the caller sent (default "sqft")
    const resolvedUnit = isOutsourced
      ? (material?.unit || "sqft")
      : isQtyProduct
        ? (product?.primary_unit || "pcs")
        : (product?.primary_unit || "sqft");
    ...
    const issue = await MaterialIssue.create({
      ...
      wastage_sqft:        wastageSq,
      calculation:         calc,
      // Store product_type and job_qty in issue_notes as JSON suffix so
      // reporting can distinguish sqft vs quantity issues without a migration.
      // Format: "<user notes> [{"pt":"quantity","jq":5}]"
      // Only appended when product_type is "quantity".
      issue_notes: isQtyProduct
        ? `${issue_notes}${issue_notes ? " " : ""}[{"pt":"quantity","jq":${parseFloat(job_qty) || 0}}]`
        : issue_notes,
      outsource_type,
    ...
    return resp(res, 201, true, `Material issued. Issue No: ${issue_no}`, {
      ...
      product_type:      isQtyProduct ? "quantity" : "sqft",
      printing_dimensions: printDim,
      media_dimensions:    mediaDim,
      wastage_sqft:      wastageSq,
      calculation:       calc,
      stock_remaining:   product
        ? parseFloat((product.stock_count - qty).toFixed(4))
        : null,
    });
  } catch (err) {
...
exports.reassignIssuedTo = async (req, res) => {
  try {
    ...
    const previousAssignee = issue.issued_to
      ? { ...issue.issued_to.toObject?.() ?? issue.issued_to }
      : null;
  ...
  try {
    ...
    if (!alreadyHasProductionData) {
      issue.applyProductionCompletion({
        ...
        production_started_at, production_completed_at, production_duration_seconds,
      });
    }
...
exports.updatePickupStatus = async (req, res) => {
  try {
    const { issueId } = req.params;
    const { status, received_by, receiver_mobile } = req.body;
    ...
    // Store receiver details on delivery
    if (status === "delivered") {
    ...
    return resp(res, 200, true, `Pickup status updated to "${status}".`, {
      ...
      received_by:   pa.received_by   || null,
      receiver_mobile: pa.receiver_mobile || null,
    });
  } catch (err) {
...
exports.getItemMaterials = async (req, res) => {
  try {
    ...
    return resp(res, 200, true, "Item material issues fetched.", {
      issues, totals: computeJobTotals(issues),
    });
  } catch (err) {
...
exports.deleteMaterialIssue = async (req, res) => {
  try {
    ...
    if (issue.status !== "issued")
      return resp(res, 400, false,
        `Cannot delete ${issue.issue_no} — only unprocessed (status=issued) records can be deleted.`
      );
    ...
    if (issue.outsource_type === "none" || !issue.outsource_type) {
      await incrementStock(
        ...
        issue.issued_qty,
        "System",
        `Reversal — deleted ${issue.issue_no}`
      );
    }
