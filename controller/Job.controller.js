...
exports.collectPaymentDelivery = async (req, res) => {
  try {
    ...
    if (pAmount < 0 || isNaN(pAmount)) {
      return resp(res, 400, false, "Payment amount must be a valid non-negative number");
    }

    if (discountAmt < 0 || isNaN(discountAmt)) {
      return resp(res, 400, false, "Discount amount must be a valid non-negative number");
    }

    if (pAmount <= 0) {
      return resp(res, 400, false, "Payment amount must be greater than 0");
    }

    if (pAmount > 0 && (!method || typeof method !== "string" || !method.trim())) {
    ...
    if (discountAmt > 0) {
      ...
      if (discountAmt > currentBalance + 0.01) {
        return resp(
          ...
          400,
          false,
          `Discount (₹${discountAmt.toFixed(2)}) exceeds current balance (₹${currentBalance.toFixed(2)})`
        );
      }

      // Reduce the job total by the discount so balance shrinks accordingly
      job.total_amount = parseFloat(Math.max(0, (job.total_amount || 0) - discountAmt).toFixed(2));
      job.discount_amount = parseFloat(((job.discount_amount || 0) + discountAmt).toFixed(2));

      if (typeof job.recomputePayments === "function") {
        job.recomputePayments();
      } else {
        job.balance_amount = Math.max(0, parseFloat(((job.total_amount || 0) - (job.payment_amount || 0)).toFixed(2)));
      }
    }

    // ─── VALIDATE AND ADD PAYMENT (IF ANY) ────────────────────────────────────
    if (pAmount > 0) {
      if (pAmount > (job.balance_amount || 0) + 0.01) {
        return resp(
          res,
          400,
          false,
          `Payment amount ₹${pAmount.toFixed(2)} exceeds remaining balance ₹${(job.balance_amount || 0).toFixed(2)}`
        );
      }
      ...
      if (typeof job.addPayment === "function") {
        ...
        const balanceAfter = Math.max(0, parseFloat(((job.balance_amount || 0) - pAmount).toFixed(2)));
        const newPayment = {
          amount: pAmount,
          method,
          paid_at: new Date(),
        ...
        job.payment_amount = parseFloat(((job.payment_amount || 0) + pAmount).toFixed(2));
        job.balance_amount = balanceAfter;

        if (job.balance_amount <= 0) {
          job.next_due_date = null;
        } else if (next_due_date) {
          job.next_due_date = new Date(next_due_date);
        }
      }
    } else {
    ...
    // ─── RESPONSE ──────────────────────────────────────────────────────────────
    const lastPayment = job.payments && job.payments.length > 0
      ? job.payments[job.payments.length - 1]
      : null;

    return resp(res, 200, true, "Payment collected successfully", {
      payment_id: lastPayment?._id?.toString() || null,
      payment_amount: pAmount,
      discount_amount: job.discount_amount || 0,
      discount_applied: discountAmt,
      tax_amount: job.tax_amount || 0,
      method: pAmount > 0 ? method : (lastPayment?.method || ""),
      paid_at: lastPayment?.paid_at || new Date(),
      subtotal: job.subtotal || 0,
      taxable_amount: job.taxable_amount || 0,
      tax_amount: job.tax_amount || 0,
      delivery_charges: job.delivery_charges || 0,
